...
%>
<aui:form action="${saveSettingsUrl}" method="POST">
	<aui:input type="hidden" name="userId" value="<%=userId%>"/>
	<table class="settingsTable">
		<tr>
		<td><%=res.getString("LangLearn.mode")%>:</td>
		<td>
			<aui:select name="mode" id="LLSetMode" label="" style="width:100%;">
			...
			<script>document.getElementById("_LangLearn_WAR_LangLearnportlet_LLSetMode").value = "<%=mode%>";</script>
		</td>
		</tr>
		<tr>
		<td><%=res.getString("LangLearn.nativeLanguage")%>:</td>
		<td>
			<aui:select name="localeFrom" id="LLSetLocaleFrom" label="" style="width:100%;">
			<%
			...
			<script>document.getElementById("_LangLearn_WAR_LangLearnportlet_LLSetLocaleFrom").value = "<%=localeFrom%>";</script>
		</td>
		</tr>
		<tr>
		<td><%=res.getString("LangLearn.languageToLearn")%>:</td>
		<td>
			<aui:select name="localeTo" id="LLSetLocaleTo" label="" style="width:100%;">
			<%
				for (Locale locale : Locale.getAvailableLocales()) {
					if (locale.getDisplayCountry().equals("")) {
			%>
			...
			<script>document.getElementById("_LangLearn_WAR_LangLearnportlet_LLSetLocaleTo").value = "<%=localeTo%>";</script>
		</td>
		</tr>
		<tr>
		<td><%=res.getString("LangLearn.wordsPerDay")%>:</td>
		<td>
			<aui:input name="wordsPerDay" label="" value="<%=wordsPerDay%>" style="width:30%; text-align:right;"/>
		</td>
		</tr>
	</table><br/><hr/>
	<div id="buttonsDiv">
